...
PREDICTION_SNAPSHOT_PATH = (
    Path(__file__).resolve().parent / 'data_collection' / 'data' / 'prediction_snapshot.json'
)
...
# Served until Jout.py pushes a live AI result.
# ─────────────────────────────────────────────────────────────────
...
def get_prediction() -> list[dict]:
    """
    Returns the current sorted prediction list.
    Serves the default until Jout.py pushes a live result via POST /prediction.
    """
...
def update_prediction(payload: list[dict]) -> dict:
    """
    Receives the full sorted prediction list from Jout.py and stores it.
