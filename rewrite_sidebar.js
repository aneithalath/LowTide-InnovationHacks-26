...
// Replaces the first panel heading and wraps the layer stack
code = code.replace(/<div className="panel-heading">[\s\S]*?<h1>Unit Tracking<\/h1>[\s\S]*?<\/div>/, `<div className="panel-heading">
          ...
          <div className="section-heading" onClick={() => setIsLayersSectionOpen(o => !o)} style={{ cursor: "pointer", marginTop: "1rem", borderBottom: "1px solid #333", paddingBottom: "4px" }}>
            <h2 style={{ fontSize: "14px", margin: 0, textTransform: "uppercase", letterSpacing: "0.05em", color: "#ccc" }}>Layers {isLayersSectionOpen ? "Gû+" : "Gû¦"}</h2>
          </div>
          
          {isLayersSectionOpen && (
            <>`);

// Wraps the end of the layer stack
code = code.replace(/<div className="panel-stats">[\s\S]*?<span>Incident Feeds<\/span>/, `  </>
          )}
